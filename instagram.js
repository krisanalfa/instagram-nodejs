  ...
  */
        ...
        if (json.status == 'ok') {
          ...
          if (json.data.user.edge_followed_by.page_info.has_next_page) {
            ...
            return new Promise((resolve) => {
              debug('fetching next page in ' + this.paginationDelay / 1000 + ' seconds');
              setTimeout(() => {
          ...
          return new Promise((resolve) => {
            debug(json);
            debug('request failed, retrying in ' + this.paginationDelay / 1000 + ' seconds');
            setTimeout(() => {
  ...
  getCsrfToken() {
    return fetch('https://www.instagram.com', {
      ...
      debug('Failed to get instagram csrf token')
    )
  }
  ...
  auth(username, password) {
    ...
    return fetch('https://www.instagram.com/accounts/login/ajax/', options).then(
      ...
      debug('Instagram authentication failed (challenge required erro)')
    )
  }
  ...
  reg(username, password, name, email) {
    ...
    return fetch('https://www.instagram.com/accounts/web_create_ajax/', {
      ...
      .catch(() => debug('Instagram registration failed'))
  }
